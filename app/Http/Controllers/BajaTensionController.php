...
// EXTRACCION DE PAGINA 1
						...
						foreach ($valuesPageOne as $v) {
							...
							if (preg_match('/^RMU:\s.+CFE$/', $v["texto"]))
								...
								if (in_array($v["texto"], $rmu_tmp1)) continue;
								else $rmu_tmp2[] = $v["texto"];
							}
							# Corte a Partir
							if (preg_match('/^CORTE\sA\sPARTIR:$/', $v["texto"])) $leftCut = $v["left"];
							...
							if (preg_match('/^\d{3}$/', $v["texto"])) $topReadTmp[] = $v["top"];
							# Tabla Fila Básico
							if (preg_match('/\bBásico\b/', $v["texto"])) $topBasic = $v["top"];
							if ($v["top"] == $topBasic) $basic[] = $v["texto"];
							# Tabla Fila Intermedio
							if (preg_match('/Intermedio/', $v["texto"])) $topInter = $v["top"];
							if ($v["top"] == $topInter) $inter[] = $v["texto"];
							# Tabla Fila Excedente
							if (preg_match('/Excedente/', $v["texto"])) $topExc = $v["top"];
							if ($v["top"] == $topExc) $exc[] = $v["texto"];
							# Tabla Fila Suma
							if (preg_match('/Suma/', $v["texto"])) $topSum = $v["top"];
							if ($v["top"] == $topSum) $sum[] = $v["texto"];
						}
						...
						foreach ($valuesPageOne as $v) {
							...
							if ($leftTotal == $v["left"] and preg_match('/^\(.+\sPESOS\s\sM.N.\)/', $v["texto"]))
								$totalLetter = $v["texto"];
							# Tabla Energia
							if ($v["top"] == $topRead) $energy[] = $v["texto"];
						}

						#RMU
						if (empty($rmu_tmp1)) $rmu = $rmu_tmp2[0];
						else {
							if ($rmu_tmp1[1] == $rmu_tmp2[0]) $rmu = $rmu_tmp2[0];
							else $rmu = $rmu_tmp2[0];
						}
// EXTRACCION DE PAGINA 1
// EXTRACCION DE PAGINA 2
						...
						foreach ($valuesPageTwo as $v) {
							if ($topPeriodos != $v["top"]) {# Periodos y kWh
								if (preg_match('/del\s(\d{2}\s[A-Z]{3}\s\d{2})\sal\s(\d{2}\s[A-Z]{3}\s\d{2})\s(\d{1,4})\s(\$\d{1,3},?\d{1,3}.00)\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"]))
									$tempPeriods[] = preg_split('/del\s(\d{2}\s[A-Z]{3}\s\d{2})\sal\s(\d{2}\s[A-Z]{3}\s\d{2})\s(\d{1,4})\s(\$\d{1,3},?\d{1,3}.00)\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"], 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
								elseif (preg_match('/del\s(\d{2}\s[A-Z]{3}\s\d{2})\sal\s(\d{2}\s[A-Z]{3}\s\d{2})\s(\d{1,4})/', $v["texto"])) {
									$tempPeriods[] = preg_split('/del\s(\d{2}\s[A-Z]{3}\s\d{2})\sal\s(\d{2}\s[A-Z]{3}\s\d{2})\s(\d{1,4})/', $v["texto"], 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
									$topPeriodos = $v["top"];
								} elseif (preg_match('/del\s(\d{2}\s[A-Z]{3}\s\d{2})\sal\s(\d{2}\s[A-Z]{3}\s\d{2})/', $v["texto"])) {
								...
								'/(\d{1,4})\s(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"])) {
								$arr = preg_split(
									'/(\d{1,4})\s(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"],
									0,
									PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
								foreach ($arr as $value) {
									$tempPeriods[count($tempPeriods) - 1][] = $value;
								}
								$topPeriodos = $v["top"];
							} elseif (preg_match('/(\d{1,4})/', $v["texto"])) {
								$arr = preg_split(
									'/(\d{1,4})/', $v["texto"],
									0,
									PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
								...
								$topPeriodos = $v["top"];
							} elseif (preg_match('/(\$\d{1,3},?\d{1,3}.00)\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"])) {
								$arr = preg_split(
									'/(\$\d{1,3},?\d{1,3}.00)\s?(\$\d{1,3},?\d{1,3}.00)?\s?(\$\d{1,3},?\d{0,3}.00)?/', $v["texto"],
									0,
									PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
								foreach ($arr as $value) {
									$tempPeriods[count($tempPeriods) - 1][] = $value;
