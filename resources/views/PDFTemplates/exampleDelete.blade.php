...
<body>
    ...
    <div class="container-fluid" style="margin-top: -25px;">
        ...
        <div class="jumbotron bordeLateral" style="border-color: #8DEB6A;">
            <h1 id="nombreCliente">{{ $cliente["vNombrePersona"] ." ". $cliente["vPrimerApellido"] ." ". $cliente["vSegundoApellido"] }}</h1>
            <p id="direccionCliente" class="subtitulo">{{ $cliente["vCalle"] ." ". $cliente["vMunicipio"] ." ". $cliente["vEstado"] }}</p>
            ...
            <table style="width: 100%; text-align: center; border-collapse: collapse; border-radius: 1em; overflow: hidden;">
                ...
                <tbody style="border:1px solid;">
                    <tr id="desglocePanel">
                        ...
                        <td>{{ $paneles["nombre"] }}</td>
                        <td>{{ $paneles["costoTotal"] }}$</td>
                    </tr>
                    <tr id="desgloceInversor">
                        <td>Inversor</td>
                        <td>{{ $inversores["vMarca"] }}</td>
                        <td>{{ $inversores["numeroDeInversores"] }}</td>
                        <td>{{ $inversores["vNombreMaterialFot"] }}</td>
                        <td>{{ $inversores["precioTotal"] }}$</td>
                    </tr>
                    <tr id="desgloceEstructura">
                        ...
                        <td>{{ $paneles["costoDeEstructuras"] }}$</td>
                    </tr>
                    <tr>
                        <td>Mano de obra</td>
                        <td>Etesla</td>
                    ...
                    <tr style="background-color: #E8E8E8;">
                        ...
                        <td align="center">${{ $totales["precio"] }} USD</td>
                    </tr>
                    <tr style="background-color: #E8E8E8;">
                        ...
                        <td align="center">${{ $totales["precioMXN"] }} MXN</td>
                    </tr>
                </tbody>
            </table>
        ...
        <table class="table-contenedor" style="margin-top: -8px;">
            <tr style="line-height: 80%;">
                ...
                <td align="center" style="background-color: #DADADA;">
                    ...
                    <p style="font-size: 18px;"><strong>{{ $paneles["potenciaReal"] }} Kwp</strong></p>
                </td>
                <td align="center">
        ...
        <div style="margin-top: 8px; margin-bottom: 25px; margin-left: 25px; margin-right: 25px;">
            <table class="tabFinanciamiento">
                <tr>
                    ...
                    <td>${{ $totales["precioMXN"] }}</td>
                    <th style="background-color: #03BABE;">Ahorro mensual<br>de luz</th>
                    <td style="background-color: #03BABE;">${{ $roi["ahorro"]["ahorroMensualEnPesosMXN"] }}</td>
                    <th>Retorno de inversión</th>
                    <td>{{ $roi["roiEnAnios"] }} años</td>
                </tr>
            </table>
            <br>
            <table class="tabFinanciamiento">
                ...
                <tr>
                    <th> Pago mensual</th>
                    <td>${{ $financiamiento["objMensualidadesCreditCard"]["tresMeses"] }}</td>
                    <td>${{ $financiamiento["objMensualidadesCreditCard"]["seisMeses"] }}</td>
                    <td>${{ $financiamiento["objMensualidadesCreditCard"]["nueveMeses"] }}</td>
                    <td>${{ $financiamiento["objMensualidadesCreditCard"]["doceMeses"] }}</td>
                    <td>${{ $financiamiento["objMensualidadesCreditCard"]["dieciochoMeses"] }}</td>
                </tr>
            </table>
            <br>
            <table id="tabFinanciamient" class="tabFinanciamiento">
                ...
                <tr>
                    <th>Enganche</th>
                    <td>${{ $financiamiento["objEnganche"]["quincePorcent"] }}</td>
                    <td>${{ $financiamiento["objEnganche"]["treintacincoPorcent"] }}</td>
                    <td>${{ $financiamiento["objEnganche"]["cincuentaPorcent"] }}</td>
                </tr>
                <tr>
    ...
    <div class="bordeLateral" style="border-color: #8DEB6A; border-top: 5px solid #FFFB00; margin-left: 32px; margin-top: 23px; text-align: center;">
        <div>
            <p>Deseamos que la propuesta solar presentada a su nombre:<br><strong>{{ $cliente["vNombrePersona"] ." ". $cliente["vPrimerApellido"] ." ". $cliente["vSegundoApellido"] }}</strong><br>sea de su agrado, quedamos a la espera de la aceptación.</p>
        </div>
        <div style="line-height: 45%;">
            <p>ASESOR: {{ $vendedor["vNombrePersona"] ." ". $vendedor["vPrimerApellido"] ." ". $vendedor["vSegundoApellido"] }}</p>
            <br>
